...
- Sets the ATmega328P temperature calibration default to `-110`, matching genuine ATmega328P devices.
- Keeps the Arducon 2.x Optiboot/STK500v1 update path, `INF` metadata, and SignalSlinger-style release package contents from v2.0.4.
...
- Hardware regression was not rerun for this calibration-default release.
...
- Updating from Arducon 1.x to 2.0.5 requires a programming device, such as an Atmel-ICE or compatible ISP programmer, because 1.x units do not already have the new Optiboot update path installed.
