...
- Improves serial command probing by accepting either CR or LF as command terminators.
- Prevents CRLF probe traffic from leaving the trailing LF as a phantom command before the next probe.
- Keeps the Arducon 2.x Optiboot/STK500v1 update path, `INF` metadata, and SignalSlinger-style release package contents from v2.0.2.
...
- Hardware regression was not rerun for this serial parser release.

## Update Notes

- Updating from Arducon 1.x to 2.0.3 requires a programming device, such as an Atmel-ICE or compatible ISP programmer, because 1.x units do not already have the new Optiboot update path installed.
